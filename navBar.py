...
class NavBar():
    ...
    with open (os.path.dirname(os.getcwd())+"//Coursework//Manager//navbuttons.txt") as f:
    ...
    def __init__(self, window, parent):
        ...
        for i in range(0, len(NavBar.NAV_BUTTONS)):
            navButton = ttk.Button(self.holdingFrame, text = NavBar.NAV_BUTTONS[i],command = lambda x=i:self.window.show_frame(x))
            navButton.grid(row = 0, column = self.c, padx = 5, pady = 5)
            self.c+=1
                       
if __name__=="__main__":
    window = tk.Tk()
    nav = NavBar(window, window)
    window.mainloop()
